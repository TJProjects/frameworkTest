...
public:
    ...
    void open(std::string server, std::string appKey, std::string appSecret, std::shared_ptr<CallbackScheduler> callbackScheduler, std::function<void(CloudStatus)> callback_open, std::function<void(CloudStatus, std::vector<std::shared_ptr<Target>>)> callback_recognize);
    bool close();
...
inline void CloudRecognizer::open(std::string arg0, std::string arg1, std::string arg2, std::shared_ptr<CallbackScheduler> arg3, std::function<void(CloudStatus)> arg4, std::function<void(CloudStatus, std::vector<std::shared_ptr<Target>>)> arg5)
{
    easyar_CloudRecognizer_open(cdata_.get(), std_string_to_easyar_String(arg0).get(), std_string_to_easyar_String(arg1).get(), std_string_to_easyar_String(arg2).get(), (arg3 == nullptr ? nullptr : arg3->get_cdata().get()), FunctorOfVoidFromCloudStatus_to_c(arg4), FunctorOfVoidFromCloudStatusAndListOfPointerOfTarget_to_c(arg5));
}
